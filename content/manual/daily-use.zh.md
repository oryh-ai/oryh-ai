...
主数据里没有的名字——项目、供应商、客户——会立刻结束这一轮：agent 直接说该记录不存在，问你是名字写错了还是记录还没建。它不会去试别的拼法、也不会拿相近的顶替；以前两分钟的填单拖到十分钟，多半就耗在这里。整单创建（工时、报销）还接受 `?validate_only=true`：跑完服务端全部校验但什么都不写，"这样提交能不能过"一次调用就知道。
