...
```bash
ORYH_LOCALE=zh                              # zh 或 en（自托管默认 en）
```

决定服务端自己写出和预置的内容的语言：出厂词汇表（假别、报销类别、付款方式等）、能力目录、系统邮件、控制台外壳。自托管部署默认英文，中文公司把它设为 `zh`。网页会跟随浏览器语言；技能和工作流定义是工作区自己的文字，作者写什么语言就是什么语言。
